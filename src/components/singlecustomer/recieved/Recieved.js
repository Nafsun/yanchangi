...
//All filtered recieved or pay customer
const ALLFILTEREDRECIEVEDORPAYCUSTOMER = gql` 
    mutation allfilteredrecieveorpaycustomer($username: String, $fromdate: String, $todate: String, $customer: String, $customeraccountno: String, $jwtauth: String){
        allfilteredrecieveorpaycustomer(username: $username, fromdate: $fromdate, todate: $todate, customer: $customer, customeraccountno: $customeraccountno, jwtauth: $jwtauth){
...
const TOTALITYFORPAYORRECIEVEDCUSTOMER = gql`
    query totalityforpayorrecievedcustomer($username: String, $customer: String, $customeraccountno: String, $jwtauth: String){
        totalityforpayorrecievedcustomer(username: $username, customer: $customer, customeraccountno: $customeraccountno, jwtauth: $jwtauth){
...
const RECEIVEORPAYGETCUSTOMER = gql`
    query recieveorpaygetcustomer($username: String, $customer: String, $customeraccountno: String, $startc: Int, $endc: Int, $jwtauth: String){
        recieveorpaygetcustomer(username: $username, customer: $customer, customeraccountno: $customeraccountno, startc: $startc, endc: $endc, jwtauth: $jwtauth){
            id, username, amount, chooseclient, recievedorpay, fromorto, bankname, bankaccountnumber, bankaccountname, accountnumber, date
        }
    }
`;
...
function Recieved(props) {
    ...
    const [allfilteredrecieveorpaycustomermutation] = useMutation(ALLFILTEREDRECIEVEDORPAYCUSTOMER);
    ...
    const [filterRecieveorPayGet, filterRecieveorPaySet] = useState([]);
    ...
    useEffect(() => { //convert then to initial values when you change page
        if (CustomerChangi[1] !== accountno_save) {
            starter2 = 0;
            ender2 = 50;
            accountno_save = CustomerChangi[1];
        }
    }, [CustomerChangi]);
    ...
    const totality = useQuery(TOTALITYFORPAYORRECIEVEDCUSTOMER,
        {
    ...
    const { error, data, refetch } = useQuery(RECEIVEORPAYGETCUSTOMER,
        {
            variables: { username: userinfo === null ? "nothing" : userinfo.loginAccount.username, customer: CustomerChangi[0], customeraccountno: CustomerChangi[1], startc: starter2, endc: ender2, jwtauth: userinfo === null ? "nothing" : userinfo.loginAccount.token },
            fetchPolicy: 'no-cache',
            onCompleted() {
                if (data !== undefined) {
                    waitloadSet(true);
    ...
    const FilterNow = () => {
        ...
        allfilteredrecieveorpaycustomermutation({ variables: { username: u, fromdate: fromdate.value, todate: todate.value, customer: CustomerChangi[0], customeraccountno: CustomerChangi[1], jwtauth: j } }).then(({ data }) => {
            nextClickSet(false);
            filterRecieveorPaySet(data.allfilteredrecieveorpaycustomer);
    ...
    return (
        <div>
                ...
                <div>
                    ...
                    <div className="workspace2">
                        <div className="jobcontainer2">
                            ...
                            {data.recieveorpaygetcustomer.length === 0 && starter2 === 0 && nextClickGet === false ? <p align="center" className="datef">{CustomerChangi[0]} have not recieved or send money yet</p> : ""}
                            {data.recieveorpaygetcustomer.length === 0 && starter2 > 0 ? <p align="center" className="datef">No More Send/Recieved Transactions</p> : ""}
                            {starter2 === 0 ? "" : <p onClick={() => PreviousBroadcast()} className="leftNav"><ArrowBack /></p>}
                            {data.recieveorpaygetcustomer.length === 0 ? "" : <p onClick={() => NextBroadcast()} className="rightNav"><ArrowForward /></p>}
                            <div className="changefloat2"></div>

                            <div className="loginspace" id="printableArea">
                                <table cellSpacing={10} align="center">
                                    {data.recieveorpaygetcustomer.length === 0 && (starter2 > 0 || starter2 === 0) ? <thead></thead> :
                                        <thead>
                                            <tr className="tablecolumdesign">
                                                <td>Amount</td>
                                                <td>Type</td>
                                    ...
                                    <tbody>
                                        {data.recieveorpaygetcustomer.map((t) => (
                                            <tr className="tablecolumdesign2" key={t.id}>
                                                <td>{Naira(t.amount)}</td>
                                                <td>{t.recievedorpay}</td>
                                                <td>{t.bankname}</td>
                    ...
                    {data.recieveorpaygetcustomer.length === 0 && (starter2 > 0 || starter2 === 0) ? "" :
                        <div>
                            <p className="tablecolumdesign">Total Amount Recieved: {waitloadGet2 === false ? "" : <span className="tablecolumdesign3">{Naira(totality.data.totalityforpayorrecievedcustomer.amountrecieved)}</span>}</p>
                            <p className="tablecolumdesign">Total Amount Pay: {waitloadGet2 === false ? "" : <span className="tablecolumdesign3">{Naira(totality.data.totalityforpayorrecievedcustomer.amountpay)}</span>}</p>
                        </div>
                    }
            ...
            {accessv.data.accessverify.createrecieveorpay === "yes" ?
                <RecieveOrPay refetchtotal={props.refetch} refetch={refetch} name={CustomerChangi[0]} accountno={CustomerChangi[1]} chooseclient={"customer"} recieved={"recieved"} totalityrefetch={totality.refetch} />
                : ""}
        </div>
    );
}
