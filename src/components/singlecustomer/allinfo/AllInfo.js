...
const BUYANDSELLGETCUSTOMER = gql`
    query buyandsellgetcustomer($username: String, $customer: String, $customeraccountno: String, $startc: Int, $endc: Int, $jwtauth: String){
        buyandsellgetcustomer(username: $username, customer: $customer, customeraccountno: $customeraccountno, startc: $startc, endc: $endc, jwtauth: $jwtauth){
...
function AllInfo(props) {
    ...
    const totality = useQuery(TOTALITYFORCUSTOMER,
        {
    ...
    const { error, data, refetch } = useQuery(BUYANDSELLGETCUSTOMER,
        {
    ...
    return (
        <div>
                ...
                <div>
                    <div className="workspace2">
                        <div className="jobcontainer2">
                            ...
                            {data.buyandsellgetcustomer.length === 0 && starter2 === 0 && nextClickGet === false ? <p align="center" className="datef">You have not sold any currency to {CustomerChangi[0]} yet</p> : ""}
                            {data.buyandsellgetcustomer.length === 0 && starter2 > 0 ? <p align="center" className="datef">No More Transactions</p> : ""}
            ...
            {accessv.data.accessverify.createtransaction === "yes" ?
                <Transaction refetchtotal={props.refetch} refetch={refetch} customername={CustomerChangi[0]} customeraccountno={CustomerChangi[1]} totalityrefetch={totality.refetch} />
                : ""}
        </div>
    );
}
