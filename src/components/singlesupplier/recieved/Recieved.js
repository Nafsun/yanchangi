...
const TOTALITYFORPAYORRECIEVEDSUPPLIER = gql`
    query totalityforpayorrecievedsupplier($username: String, $supplier: String, $supplieraccountno: String, $jwtauth: String){
        totalityforpayorrecievedsupplier(username: $username, supplier: $supplier, supplieraccountno: $supplieraccountno, jwtauth: $jwtauth){
...
const RECEIVEORPAYGETSUPPLIER = gql` 
    query recieveorpaygetsupplier($username: String, $supplier: String, $supplieraccountno: String, $startc: Int, $endc: Int, $jwtauth: String){
        recieveorpaygetsupplier(username: $username, supplier: $supplier, supplieraccountno: $supplieraccountno, startc: $startc, endc: $endc, jwtauth: $jwtauth){
...
function Recieved(props) {
    ...
    const totality = useQuery(TOTALITYFORPAYORRECIEVEDSUPPLIER,
        {
    ...
    const { error, data, refetch } = useQuery(RECEIVEORPAYGETSUPPLIER,
        {
    ...
    return (
        <div>
                ...
                <div>
                    <div className="workspace2">
                        <div className="jobcontainer2">
                            ...
                            {data.recieveorpaygetsupplier.length === 0 && starter2 === 0 && nextClickGet === false ? <p align="center" className="datef">{SupplierChangi[0]} have not recieved or send money yet</p> : ""}
                            {data.recieveorpaygetsupplier.length === 0 && starter2 > 0 ? <p align="center" className="datef">No More Send/Recieved Transactions</p> : ""}
                    ...
                    {data.recieveorpaygetsupplier.length === 0 && (starter2 > 0 || starter2 === 0) ? "" :
                        <div>
                            <p className="tablecolumdesign">Total Amount Recieved: {waitloadGet2 === false ? "" : <span className="tablecolumdesign3">{Naira(totality.data.totalityforpayorrecievedsupplier.amountrecieved)}</span>}</p>
                            <p className="tablecolumdesign">Total Amount Pay: {waitloadGet2 === false ? "" : <span className="tablecolumdesign3">{Naira(totality.data.totalityforpayorrecievedsupplier.amountpay)}</span>}</p>
                        </div>
                    }
            ...
            {accessv.data.accessverify.createrecieveorpay === "yes" ?
                <RecieveOrPay refetchtotal={props.refetch} refetch={refetch} name={SupplierChangi[0]} accountno={SupplierChangi[1]} chooseclient={"supplier"} recieved={"pay"} totalityrefetch={totality.refetch} />
                : ""}
        </div>
    );
}
